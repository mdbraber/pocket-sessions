...
extension CarPlaySceneDelegate {
    ...
    func downloadsTapped() {
        pushEpisodeList(title: L10n.downloads, emptyTitle: L10n.downloadsNoDownloadsTitle, showArtwork: true, playlist: .downloads) { () -> [BaseEpisode] in
            DataManager.shared.findEpisodesWhere(customWhere: "episodeStatus == \(DownloadStatus.downloaded.rawValue) ORDER BY lastDownloadAttemptDate DESC LIMIT \(Constants.Limits.maxCarplayItems)", arguments: nil)
        }
    }
