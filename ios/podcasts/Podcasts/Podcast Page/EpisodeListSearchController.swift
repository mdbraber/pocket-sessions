...
class EpisodeListSearchController: SimpleNotificationsViewController, UITextFieldDelegate {
    ...
    /// Hides the info row and gives back the space it occupies
    var isInfoRowCollapsed = false {
        didSet {
            guard isInfoRowCollapsed != oldValue else { return }
            updateInfoRowCollapsed()
        }
    }
    ...
    /// Applies custom styling to the action button after its title is set
    func styleActionButton(_ style: (UIButton) -> Void) {
        guard let actionButton, actionTitle != nil else { return }
    ...
    private lazy var infoRowZeroHeights: [NSLayoutConstraint] = [infoLabel, actionButton]
        .compactMap { $0?.heightAnchor.constraint(equalToConstant: 0) }

    /// The info row's top spacing and height give this view its height, so hiding the views
    /// alone would leave an empty row. Their spacing is zeroed too, and restored exactly.
    private func updateInfoRowCollapsed() {
        guard let infoLabel, let container = infoLabel.superview else { return }

        if infoRowSpacing.isEmpty {
            let block: [UIView?] = [infoLabel, actionButton]
        ...
        for entry in infoRowSpacing {
            entry.constraint.constant = isInfoRowCollapsed ? 0 : entry.original
        }
        for constraint in infoRowZeroHeights {
            constraint.isActive = isInfoRowCollapsed
        }
        infoLabel.isHidden = isInfoRowCollapsed
        view.layoutIfNeeded()
    }
