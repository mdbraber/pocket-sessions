...
final class InboxPlaylistFoundationsTests: DataManagerTestCase {
    ...
    func testInboxPlaylistIsHiddenFromEveryEnumerationAPI() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertFalse(all.contains(DataManager.inboxPlaylistUuid), "allPlaylists must not expose the Inbox")
            XCTAssertFalse(manual.contains(DataManager.inboxPlaylistUuid), "allManualPlaylists must not expose the Inbox")
            XCTAssertFalse(smart.contains(DataManager.inboxPlaylistUuid), "allSmartPlaylists must not expose the Inbox")
            XCTAssertEqual(all.count, 2, "the two real playlists still show")
            XCTAssertEqual(dataManager.playlistsCount(includeDeleted: false), 2, "the count excludes the Inbox too")
        }
    }
    ...
    func testInboxPlaylistStillSyncsAndIsStillFindable() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertTrue(unsynced.contains(DataManager.inboxPlaylistUuid), "the Inbox MUST still be picked up for sync")
            XCTAssertNotNil(dataManager.findPlaylist(uuid: DataManager.inboxPlaylistUuid), "the fork must still be able to fetch the Inbox itself")
        }
    }
    ...
    func testPlaylistEpisodeUuidsReturnsMembershipWithoutHydratingEpisodes() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.playlistEpisodeUuids(for: playlist.uuid), ["e1", "e2", "e3"])
        }
    }

    func testPlaylistEpisodeUuidsIsEmptyForAnUnknownPlaylist() throws {
        try runWithDataManager { dataManager in
            XCTAssertTrue(dataManager.playlistEpisodeUuids(for: "nope").isEmpty)
        }
    }
    ...
    func testApplyEpisodeOrderReordersMembership() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.positionedEpisodeUuids(for: playlist), ["d", "b", "a", "c"])
        }
    }
    ...
    func testApplyEpisodeOrderIsANoOpWhenTheOrderAlreadyMatches() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.positionedEpisodeUuids(for: playlist), ["a", "b", "c"])
        }
    }
    ...
    func testApplyEpisodeOrderSinksUnrankedRowsToTheBottomInOrder() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.positionedEpisodeUuids(for: playlist), ["c", "a", "b", "d"])
        }
    }
    ...
    func testDeletingAnEpisodeCascadesToPlaylistMembership() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.playlistEpisodeUuids(for: playlist.uuid), ["survivor"],
                           "a deleted episode must not leave an orphan membership row behind")
        }
    }
    ...
    func testDeletingAnEpisodeLeavesTheUpNextQueueAlone() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextEpisodes().map(\.uuid), ["queued"],
                           "deleting an unrelated episode must not disturb Up Next")
        }
    }
    ...
    func testDeletingAllEpisodesInAPodcastCascadesToPlaylistMembership() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.playlistEpisodeUuids(for: playlist.uuid), ["kept"])
        }
    }
    ...
    func testAddingEpisodesMarksThePlaylistUnsynced() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(reloaded.syncStatus, SyncStatus.notSynced.rawValue,
                           "a membership change must mark the playlist dirty, or it never syncs")
        }
    }
    ...
    func testExtraWhereIsAndedOntoThePlaylistQuery() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(Set(unfiltered), ["wanted", "unwanted"], "sanity — both match the bare playlist")

            let filtered = dataManager.playlistEpisodes(for: playlist, matching: "episode.uuid = 'wanted'").map(\.uuid)
            XCTAssertEqual(filtered, ["wanted"], "the preset fragment must AND onto the playlist's own rules")
        }
    }
}
