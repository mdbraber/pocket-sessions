...
final class PodcastColumnConsistencyTests: DataManagerTestCase {
    ...
    func testSettingsAreEncodedAsJSONInSettingsColumn() throws {
        let podcast = createFullyPopulatedPodcast()
        podcast.settings.autoStartFrom = 42

        let encoded = try podcast.databaseDictionary
        let json = try XCTUnwrap(String.fromDatabaseValue(try XCTUnwrap(encoded["settings"])))
        let decoded = try JSONDecoder().decode(PodcastSettings.self, from: Data(json.utf8))

        XCTAssertEqual(decoded.autoStartFrom, 42)
        XCTAssertNil(encoded["settingsJSON"])
    }
    ...
    func testTransientPropertiesAreNotEncoded() throws {
        ...
        for name in ["cachedUnreadCount", "forceRefreshEpisodeFrom"] {
