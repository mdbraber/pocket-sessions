...
func (s *Store) AllEpisodeProgress(userID int64) (map[string]EpisodeProgress, error) {
	rows, err := s.db.Query(`
SELECT episode_uuid, podcast_uuid, played_up_to, playing_status, duration
FROM episode_progress WHERE user_id = ?`, userID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.EpisodeUUID, &p.PodcastUUID, &p.PlayedUpTo, &p.PlayingStatus, &p.Duration); err != nil {
			return nil, err
		}
		out[p.EpisodeUUID] = p
	}
...
func (s *Store) SaveEpisodeProgress(userID int64, entries []EpisodeProgress) error {
	...
	for _, p := range entries {
		if _, err := tx.Exec(`
INSERT INTO episode_progress (user_id, episode_uuid, podcast_uuid, played_up_to, playing_status, duration, updated_at)
VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
...
SET podcast_uuid = excluded.podcast_uuid, played_up_to = excluded.played_up_to,
    ...
    updated_at = excluded.updated_at`,
			userID, p.EpisodeUUID, p.PodcastUUID, p.PlayedUpTo, p.PlayingStatus, p.Duration); err != nil {
			return err
		}
	}
	return tx.Commit()
